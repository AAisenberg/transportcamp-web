...
export function SocialLinks({ variant = 'dark', className = '' }: SocialLinksProps) {
  ...
  const colour =
    variant === 'light'
      ? 'text-tc-muted hover:text-tc-orange'
      ...
      {items.map((item) => (
        <li key={item.label}>
          <a
            ...
            className={`inline-flex items-center gap-2 font-sans text-sm transition-colours ${colour}`}
            aria-label={`TransportCamp on ${item.label}`}
          >
            {item.icon}
